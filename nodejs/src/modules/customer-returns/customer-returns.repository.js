...
async function validateReturnQuantities(tx, returnId, invoiceId, items) {
  if (!invoiceId) return [];
  ...
  let sql = `
    ...
    WHERE cr.invoice_id = ? AND cr.delete_flg = 0 AND cr.status <> 'rejected'
  `;
...
export class CustomerReturnsRepository {
  async findAll({
    ...
    const dataSql = `
      SELECT cr.*, c.name as customer_name, i.invoice_number,
             (SELECT COUNT(*) FROM customer_return_items cri WHERE cri.customer_return_id = cr.id) as items_count
      ${sql}
  ...
  async findById(id) {
    const rows = await query(`
      ...
      JOIN customers c ON cr.customer_id = c.id
      LEFT JOIN invoices i ON cr.invoice_id = i.id
      WHERE cr.id = ? AND cr.delete_flg = 0
      LIMIT 1
    `, [id]);
    ...
    const items = await query(`
      ...
      JOIN product_variants pv ON cri.product_variant_id = pv.id
  ...
  async countAll() {
    const rows = await query("SELECT COUNT(*) as count FROM customer_returns");
    return rows[0].count;
  }

  async resolveValidatedItems(returnId, invoiceId, items) {
    return transaction(async (tx) => {
      const invoiceItems = await validateReturnQuantities(tx, returnId, invoiceId, items);
    ...
    return transaction(async (tx) => {
      const [result] = await tx.execute(`
        INSERT INTO customer_returns (
          rma_number, customer_id, invoice_id, request_date, reason, disposition,
      ...
      for (const item of items) {
        await tx.execute(`
          INSERT INTO customer_return_items (
            customer_return_id, product_id, product_variant_id, product_name, variant_name,
  ...
  async updateResolvedReturn(id, data) {
    const { items, ...returnData } = data;
    return transaction(async (tx) => {
      if (items) {
        await tx.execute("DELETE FROM customer_return_items WHERE customer_return_id = ?", [id]);
        for (const item of items) {
          await tx.execute(`
            INSERT INTO customer_return_items (
              customer_return_id, product_id, product_variant_id, product_name, variant_name,
      ...
      await tx.execute(`
        UPDATE customer_returns SET
          customer_id = ?, invoice_id = ?, request_date = ?, reason = ?, disposition = ?,
  ...
  async approveReturn(id, context = {}) {
    ...
    return transaction(async (tx) => {
      const [returns] = await tx.execute("SELECT * FROM customer_returns WHERE id = ? AND delete_flg = 0", [id]);
      ...
      const [items] = await tx.execute("SELECT * FROM customer_return_items WHERE customer_return_id = ?", [id]);
      ...
      for (const item of items) {
        ...
        const currentQty = variantRows[0].stock_quantity;
        const newQty = currentQty + item.quantity;
        await tx.execute(
          "UPDATE product_variants SET stock_quantity = ? WHERE id = ?",
          [newQty, item.product_variant_id]
        );
      ...
      if (remainingCredit > 0 && returnRecord.invoice_id) {
        const [receivableRows] = await tx.execute(
          "SELECT * FROM accounts_receivable WHERE invoice_id = ? AND delete_flg = 0",
          ...
          if (receivable.status !== "paid") {
            ...
            await tx.execute(`
              INSERT INTO return_allocations (
                customer_return_id, accounts_receivable_id, amount_allocated, created_ip, updated_ip
              ) VALUES (?, ?, ?, ?, ?)
            `, [id, receivable.id, toApply, ipAddress, ipAddress]);
      ...
      if (remainingCredit > 0) {
        const [unpaidReceivables] = await tx.execute(`
          ...
          WHERE customer_id = ? AND delete_flg = 0 AND status IN ('unpaid', 'partial')
      ...
      await tx.execute(
        "UPDATE customer_returns SET status = 'completed', notes = ?, updated_ip = ? WHERE id = ?",
        [notes, ipAddress, id]
      );
  ...
  async rejectReturn(id, reason, context = {}) {
    const ipAddress = context.ipAddress || null;
    const returns = await query("SELECT * FROM customer_returns WHERE id = ?", [id]);
    ...
    await query(
      "UPDATE customer_returns SET status = 'rejected', notes = ?, updated_ip = ? WHERE id = ?",
      [notes, ipAddress, id]
    );
    return { success: true };
  }
  ...
  async softDelete(id) {
    await query("UPDATE customer_returns SET delete_flg = 1 WHERE id = ?", [id]);
  }

  async findCustomerInvoices(customerId) {
    const invoices = await query(`
      SELECT i.*
      FROM invoices i
      WHERE i.customer_id = ? AND i.delete_flg = 0
