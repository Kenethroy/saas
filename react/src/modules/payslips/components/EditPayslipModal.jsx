...
import { updatePayslip } from "@/modules/payslips/api/payslips.api";
...
export function EditPayslipModal({ show, payslip, employees = [], onClose }) {
  ...
  useEffect(() => {
    if (!show || !payslip) return;

    setForm({
      employeeId: payslip.employee_id != null ? String(payslip.employee_id) : "",
      periodStart: formatDateInput(payslip.period_start),
      periodEnd: formatDateInput(payslip.period_end),
      payDate: formatDateInput(payslip.pay_date),
      basicPay: String(payslip?.metadata?.basic_pay ?? 0),
      overtimePay: String(payslip?.overtime_pay ?? 0),
      allowances: String(payslip?.metadata?.allowances ?? 0),
      deductions: String(payslip?.metadata?.deductions ?? 0),
      notes: payslip.notes ?? ""
    });
  }, [show, payslip]);
  ...
  const mutation = useMutation({
    mutationFn: (payload) => updatePayslip(payslip.id, payload),
    onSuccess: async () => {
    ...
    onError: (error) => {
      notify.error(error?.response?.data?.message || "Failed to update payslip");
    }
  });
  ...
  async function handleSubmit() {
    ...
    if (String(payslip.status || "draft") !== "draft") {
      return notify.warning("Only draft payslips can be edited.");
    }
  ...
  return (
    <FormModal show={show} title="Edit Payslip" size="2xl" onClose={onClose} footer={footer}>
      <div className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div>
            <label className="erp-label">Payslip No.</label>
            <input className="erp-input" value={payslip?.payslip_number ?? ""} disabled readOnly />
          </div>
        </div>
